 ...
 */
public class Blocks_3 extends Blocks {
    ...
    public Blocks_3(World world, float x, float y,Bloxet game,boolean infinity) {
        super(world, x, y,game,infinity);

        this.game=game;
        this.infinity=infinity;

        texture=game.assetManager.get("block_3.png");
        blocksRegion=new TextureRegion(texture,0,0,67,67);

        sensorsSetToFalse();

        Blocks_3_array.delete_specific_body.add(this);
                ...
                if (y_down==18)
                {
                    ...
                    else
                    {
                        if (Grid.grid[y_down][x_movement + 1] == 0)
                        {
                            ...
                            Grid.grid[y_down][x_movement+1]=3;
    ...
    public void collision_detection(){
        ...
        //SQUARE
        if(Grid.grid[y_down][x_movement]==3 && (x_movement<10 &&(Grid.grid[y_down][x_movement+1]==3))
                && (y_down<18 &&(Grid.grid[y_down+1][x_movement]==3)) && (x_movement<10 && y_down<18 &&(Grid.grid[y_down+1][x_movement+1]==3)))
        {
            deleteBlock=true;
        }

        //VERTICAL
        else if ((y_down<15) && (Grid.grid[y_down][x_movement] == 3 && Grid.grid[y_down+1][x_movement] == 3
                && Grid.grid[y_down+2][x_movement] == 3 && Grid.grid[y_down+3][x_movement] == 3))
        {
            deleteBlock=true;
        }
