 ...
 */
public class Blocks_1 extends Blocks {
    ...
    public Blocks_1(World world, float x, float y,Bloxet game,boolean infinity)
    {
        ...
        texture=game.assetManager.get("block_1.png",Texture.class);
        blocksRegion=new TextureRegion(texture,0,0,67,67);

        sensorsSetToFalse();

        Blocks_1_array.delete_specific_body.add(this);
                ...
                if (y_down==18)
                {
                    ...
                    else
                    {
                        if (Grid.grid[y_down][x_movement + 1] == 0)
                        {
                            ...
                            Grid.grid[y_down][x_movement+1]=1;
    ...
    private void collision_detection(){
            //SQUARE
            if(Grid.grid[y_down][x_movement]==1 && (x_movement<10 &&(Grid.grid[y_down][x_movement+1]==1))
                    && (y_down<18 &&(Grid.grid[y_down+1][x_movement]==1)) && (x_movement<10 && y_down<18 &&(Grid.grid[y_down+1][x_movement+1]==1)))
            {
                deleteBlock=true;
            }

            //VERTICAL
            else if ((y_down<15) && (Grid.grid[y_down][x_movement] == 1 && Grid.grid[y_down+1][x_movement] == 1
                    && Grid.grid[y_down+2][x_movement] == 1 && Grid.grid[y_down+3][x_movement] == 1))
            {
                deleteBlock=true;
            }
    }
